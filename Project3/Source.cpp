...
string output = "out sio2 2500 particles di 20 dist 30 v5 1MHzmnmn.txt";//output file name
...
double Upfde(double Es, double CMf)
{
	double landa;// , EE0;
...
int main() {
	...
	if (plot_dep_enrgy == false) {
		for (i = 0; i < Pnum; i++) {
			Ps[i].x[0] = (double)fRand(0., d_lx);
			Ps[i].y[0] = (double)fRand(0., d_ly);
			...
			Ps[i].z[0] = (double)fRand(z0 + 0.0001, d_lz);
			...
			for (k = (i - 1); k >= 0; k--) {
				...
				int OverlapChance;
				A = calcDistance(Ps[i].x[0], Ps[k].x[0], Ps[i].y[0], Ps[k].y[0], Ps[i].z[0], Ps[k].z[0], d_lx, d_ly);
		...
		for (k = 0; k < Pnum; k++) {
			...
			for (ke = 0; ke < Pnum; ke++) {
				...
				A = calcDistance(Ps[k].x[0], Ps[ke].x[0], Ps[k].y[0], Ps[ke].y[0], Ps[k].z[0], Ps[ke].z[0], d_lx, d_ly);
			...
			A2 = calcDistance(Ps[k].x[0], (d_lx / 2.) - bb, Ps[k].y[0], (d_ly / 2.) - bb, Ps[k].z[0], Ps[k].z[0], d_lx, d_ly);
			A3 = calcDistance(Ps[k].x[0], (d_lx / 2.) - bb, Ps[k].y[0], (d_ly / 2.) + bb, Ps[k].z[0], Ps[k].z[0], d_lx, d_ly);
			A4 = calcDistance(Ps[k].x[0], (d_lx / 2.) + bb, Ps[k].y[0], (d_ly / 2.) - bb, Ps[k].z[0], Ps[k].z[0], d_lx, d_ly);
			A5 = calcDistance(Ps[k].x[0], (d_lx / 2.) + bb, Ps[k].y[0], (d_ly / 2.) + bb, Ps[k].z[0], Ps[k].z[0], d_lx, d_ly);
		...
		for (j = 0; j < jmax; j++) {
			...
			for (i = 0; i < Pnum; i++) {
				ff = 0;
				if (i != randi) {
					A = calcDistance(Ps[randi].x[1], Ps[i].x[0], Ps[randi].y[1], Ps[i].y[0], Ps[randi].z[1], Ps[i].z[0], d_lx, d_ly);
					//B = 1.05*Di; we make it dimensionless
					B = 1.005;
					if (A > B) {
						OverlapChance = false;
					...
					if (o == (Pnum - 1)) {
						...
						for (ke = 0; ke < Pnum; ke++) {
							if (ke != randi) {
								A = calcDistance(Ps[randi].x[1], Ps[ke].x[1], Ps[randi].y[1], Ps[ke].y[1], Ps[randi].z[1], Ps[ke].z[1], d_lx, d_ly);
								B = paticles_cut_off_distance;
								// new added: we need to add A > 340 because if there's a overlab the energy of system will increase to infinity						
								if (A <= B) {
									//G += EvdwSS(A) + ESt_SR_SR(A) + ESt_SR_ER(A) + ESt_ER_ER(A);
						...
						A1 = calcDistance(Ps[randi].x[1], (d_lx / 2.) - 0, Ps[randi].y[1], (d_ly / 2.) - 0, Ps[randi].z[1], Ps[randi].z[1], d_lx, d_ly);
						A2 = calcDistance(Ps[randi].x[1], (d_lx / 2.) - bb, Ps[randi].y[1], (d_ly / 2.) - bb, Ps[randi].z[1], Ps[randi].z[1], d_lx, d_ly);
						A3 = calcDistance(Ps[randi].x[1], (d_lx / 2.) - bb, Ps[randi].y[1], (d_ly / 2.) + bb, Ps[randi].z[1], Ps[randi].z[1], d_lx, d_ly);
						A4 = calcDistance(Ps[randi].x[1], (d_lx / 2.) + bb, Ps[randi].y[1], (d_ly / 2.) - bb, Ps[randi].z[1], Ps[randi].z[1], d_lx, d_ly);
						A5 = calcDistance(Ps[randi].x[1], (d_lx / 2.) + bb, Ps[randi].y[1], (d_ly / 2.) + bb, Ps[randi].z[1], Ps[randi].z[1], d_lx, d_ly);
						A6 = calcDistance(Ps[randi].x[1], 0, Ps[randi].y[1], 0, Ps[randi].z[1], Ps[randi].z[1], d_lx, d_ly);
